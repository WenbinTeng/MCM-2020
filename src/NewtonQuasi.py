# Optimization Method: Quasi Newton Method
...
def Newton_Qua(f_sym,x_sym,x_ini):

    e = 1e-6
    x_del = np.array(x_ini,dtype=np.float32)
    x_rec = np.array(x_ini,dtype=np.float32)
    x_cur = np.array(x_ini,dtype=np.float32)
    g_del = np.ones(len(x_ini))
    g_val = np.ones(len(x_ini))
    g_sym = [diff(f_sym, x) for x in x_sym]
    u_val = np.eye(len(x_ini))
    ...
    while gen < gen_max:
        ...
        x_del = x_cur
        x_cur = x_cur - np.dot(u_val,g_val) * search_1d(f_sym,x_sym,x_cur,g_val,u_val)
        x_rec = np.append(x_rec, x_cur).reshape(-1,2)
        x_del = x_cur - x_del
        x_del = x_del.reshape(-1,1)
        ...
        g_del = g_val - g_del
        g_del = g_del.reshape(-1,1)

        u_val = (
            u_val
          + np.dot(x_del,x_del.T)
          / np.dot(g_del.T,x_del)
          - np.dot(np.dot(np.dot(u_val,g_del),g_del.T),u_val)
          / np.dot(np.dot(g_del.T,u_val),g_del)
        )
...
def search_1d(y_sym,x_sym,x_val,g_val,u_val):
    Lambda = 0
    min_value = inf
    min_index = 0
    search_step = 1
    ...
    while search_step >= 1e-3:
        ...
        while True:
            x_tmp = x_val - np.dot(u_val,g_val) * Lambda
...
if __name__ == "__main__":
    ...
    x_sym = [x1,x2]
    x_ini = [1,0.5]
    f_val,x_cur,x_rec = Newton_Qua(f,x_sym,x_ini)
